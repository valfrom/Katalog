...
@onready var option_labels: Array = $MenuContainer/Options.get_children()
...
func _update_selection() -> void:
        for i in range(option_labels.size()):
                ...
                if i == _current_index:
                        label.text = "< %s >" % option_text
                else:
                        label.text = option_text

        status_label.text = "Use < > or Up / Down to choose and press confirm"
